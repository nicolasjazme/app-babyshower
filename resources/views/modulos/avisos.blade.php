...
<div class="space-y-8">
    ...
    <div class="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm">
        ...
        <div id="contenedor-avisos" class="space-y-4 max-w-3xl">
            <div class="text-center py-8 text-slate-400">
                <i class="fa-solid fa-circle-notch fa-spin mr-2"></i> Cargando avisos...
            </div>
        </div>
    </div>
</div>

{{-- Modal para Redactar Anuncio --}}
<div id="modal-nuevo-aviso" class="hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl border border-slate-100 relative animate-in fade-in zoom-in-95 duration-200">
        ...
        {{-- Formulario interceptado por JS --}}
        <form id="form-aviso" onsubmit="guardarDato(event)" class="space-y-4">
            <div>
                <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Título del Aviso *</label>
                <input type="text" id="input-titulo" required placeholder="Ej. ¡Cambio de Salón!, Traer traje de baño, Estacionamiento"
            ...
            <div>
                <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Nivel de Prioridad *</label>
                <select id="input-prioridad" required
            ...
            <div>
                ...
                <textarea id="input-contenido" required rows="4" placeholder="Escribe de forma clara y concisa las instrucciones para tus invitados..."
...
<script>
    ...
    const API_URL = `/api/eventos/${EVENTO_ID}/avisos`;
    ...
    function toggleModal(id, show) {
        ...
        if (show) {
            ...
            document.getElementById('form-aviso').reset(); // Limpiamos el form al cerrar
        }
    }
    ...
    function cargarDatos() {
        if(!EVENTO_ID) {
            document.getElementById('contenedor-avisos').innerHTML = `<p class="text-rose-500 font-bold py-4">Error: No hay un evento activo seleccionado.</p>`;
            return;
        }

        fetch(API_URL)
            .then(res => res.json())
            .then(data => renderizarAvisos(data))
            .catch(err => {
                console.error("Error cargando avisos:", err);
                document.getElementById('contenedor-avisos').innerHTML = `<p class="text-slate-400 py-4">No se pudieron cargar los anuncios.</p>`;
            });
    }

    // --- DIBUJAR AVISOS EN PANTALLA ---
    function renderizarAvisos(avisos) {
        ...
        // HTML Generado dinámicamente con los colores y estilos exactos
        container.innerHTML = avisos.map(aviso => {
            const esUrgente = aviso.prioridad === 'urgente';
            const bgColor = esUrgente ? 'bg-amber-50/60 border-amber-200' : 'bg-slate-50/60 border-slate-100';
            
            const badgeUrgente = `<span class="inline-flex items-center gap-1 text-[9px] font-black px-2 py-0.5 bg-amber-500 text-white rounded-md uppercase tracking-wider animate-pulse">🚨 Urgente</span>`;
            const badgeNormal = `<span class="inline-flex items-center gap-1 text-[9px] font-black px-2 py-0.5 bg-slate-200 text-slate-700 rounded-md uppercase tracking-wider">📌 Informativo</span>`;

            return `
                <div class="p-5 rounded-2xl border transition-all flex flex-col sm:flex-row sm:items-start justify-between gap-4 ${bgColor}">
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2">
                            ${esUrgente ? badgeUrgente : badgeNormal}
                            <span class="text-[10px] text-slate-400 font-semibold">${aviso.fecha || 'Reciente'}</span>
                        </div>
                        
                        <h3 class="font-bold text-slate-800 text-base">${aviso.titulo || 'Sin título'}</h3>
                        <p class="text-xs text-slate-600 leading-relaxed font-medium">${aviso.contenido || aviso.mensaje || ''}</p>
                    </div>

                    <div class="shrink-0 self-end sm:self-start">
                        <button onclick="eliminarDato('${aviso._id}')" class="p-2 text-slate-400 hover:text-rose-500 transition-colors" title="Eliminar anuncio">
    ...
    function guardarDato(e) {
        ...
        const payload = {
            ...
            contenido: document.getElementById('input-contenido').value,
            fecha: new Date().toLocaleDateString() // Generamos una fecha básica
        };
        ...
        .then(res => res.json())
        .then(data => {
            toggleModal('modal-nuevo-aviso', false);
            cargarDatos(); // Recargar la lista
            Swal.fire({
                icon: 'success',
                title: '¡Publicado!',
                text: 'El anuncio está ahora en el tablón.',
    ...
    // --- ELIMINAR AVISO (DELETE) ---
    function eliminarDato(id) {
        Swal.fire({
            title: '¿Quitar este comunicado?',
            text: "Desaparecerá de la vista de todos los invitados.",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                .then(() => {
                    cargarDatos(); // Recargar la lista visual
                    Swal.fire({
                        icon: 'success',
                        title: 'Eliminado',
                        text: 'El anuncio fue retirado del tablón.',
